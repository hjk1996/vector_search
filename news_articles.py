...
class NewsArticles:
    ...
    def __init__(self, model, tokenizer, articles: List[NewsArticle], news_embedding: torch.Tensor) -> None:
        self.model = model
    ...
    def get_cos_sim(self, article_index: int) -> float:
        return torch.cosine_similarity(self.news_embedding[article_index], self.news_embedding, dim=2).mean(1)
    ...
    def search_n_related_articles(self, search_text: str, n: int) -> List[NewsArticle]:
        ...
        cos_sim = torch.cosine_similarity(search_emb, self.news_embedding, dim=2).mean(1)
        _, indices = torch.sort(cos_sim, descending=True)
...
if __name__ == "__main__":
    ...
    news_articles = NewsArticles(model, tokenizer, articles, news_emb)
    pprint(news_articles.search_n_related_articles("virus spreads all over the world", 10))